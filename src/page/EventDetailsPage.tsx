...
const EventDetailsPage = () => {
  ...
  const seatCount = event.building.sectors.reduce(
    (acc, s) => acc + s.rows.reduce((rAcc, row) => rAcc + row.seats.length, 0),
    0
  );
  ...
  return (
    <Box p={4}>
      ...
      <Typography>
        ...
        {event.building.address.postalCode} {event.building.address.city},{" "}
      ...
      <Box mt={4}>
        <Button
          ...
          sx={{
            ...
            borderRadius: 4,
            textTransform: "none",
            backgroundImage: "linear-gradient(45deg,rgb(66, 249, 255),rgb(40, 62, 234))", // Zielony gradient
